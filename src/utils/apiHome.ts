...
export const apiGetDashboardListCode = async (callBack?: (res: any) => void, setLoading?: (loading: boolean) => void) => {
    ...
    const _getInfo = (i: number, type: "label" | "icon" | "background") => {
        ...
        switch (i) {
            case 0:
                _result = (type === "label" ? "Khách hàng" : (type === "icon" ? "groups" : APP_COLOR.BG_DARKRED));
                break;
            case 1:
                _result = (type === "label" ? "Hợp đồng" : (type === "icon" ? "note" : APP_COLOR.BLUE));
                break;
            case 2:
                _result = (type === "label" ? "Lịch hẹn" : (type === "icon" ? "calendar-month" : APP_COLOR.BG_ORANGE));
                break;
            case 3:
                _result = (type === "label" ? "Đang hỗ trợ" : (type === "icon" ? "support-agent" : APP_COLOR.GREEN));
                break;
        }
        return _result;
    }
